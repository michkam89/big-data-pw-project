...
#| echo: false
#| label: fig-ryc2
#| fig-cap: "Liczba postow w czasie z podzialem na pytania i odpowiedzi. Lewy panel wskazuje surowe wartości liczby postów, prawy wartości logarytmiczne o podstawie 10."
...
Stosunek ilości odpowiedzi do zadawanych pytań rośnie w czasie, lecz ma na to wpływ zmniejszająca się ilość zadawanych pytań (maleje próba badana) co zostało przedstawione na @fig-ryc3. Patrząc na wartości średnie tylko co drugie pytanie uzyskiwało odpowiedź (ratio 0.48 +/- 0.22). Ogólne statystyki stosunku pytań do dopowiedzi w czasie zostały przedstawione w @tbl-ratios.
...
time_to_accept_pd = (
    time_to_accept
        ...
        .withColumn(
            "is_outlier", 
            f.when(f.col("time_to_accept_min") > 100000, True).otherwise(False)
            )).toPandas()
...
#| echo: false
#| label: fig-ryc10
#| fig-cap: Rozkład czasu od pojawienia się pytania do zaakceptowanej odpowiedzi
...
#| tbl-cap: Statystyki ocen odpowiedzi zaakceptowanych w stosunku do pozostałych po odrzuceniu wartości odstających > 100000
...
#| echo: false
#| label: tbl-q_stats
#| tbl-cap: Statystyki długości postów
...
g6_log = g6 + scale_x_log10() + scale_y_log10()

p6 = pw.load_ggplot(g6, (10,3))
p6_log = pw.load_ggplot(g6_log, (10,3))
```
...
g5_log = g5 + scale_x_log10() + scale_y_log10()

p5 = pw.load_ggplot(g5, (10,3))
p5_log = pw.load_ggplot(g5_log, (10,3))
```
...
#| echo: false
#| label: fig-ryc5
#| fig-cap: Oceny pytań na forum w zależności od długości zadanego pytania
...
#| tbl-cap: Statystyki długości postów z podziałem na podgrupy najlepszych i najgorszych pytań
...
#| tbl-cap: Statystyki ocen użytkowników z podziałem na podgrupy najlepszych i najgorszych pytań
...
#| echo: false
#| label: fig-ryc6
#| fig-cap: Oceny pytań na forum w zależności od ilości wyświetleń i odpowiedzi
...
#| tbl-cap: Statystyki ilości odpowiedzi na pytania z podziałem na podgrupy najlepszych i najgorszych pytań
edge_questions.groupby('type')\
    .agg(
...
#| tbl-cap: Statystyki ilości wyświetleń pytań z podziałem na podgrupy najlepszych i najgorszych pytań
edge_questions.groupby('type')\
    .agg(
