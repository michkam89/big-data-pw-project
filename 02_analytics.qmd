...
## Analiza aktywności użytkowników na forum {#sec-activity}
...
```
\newpage
## Dynamika oraz statystyki udzielanych odpowiedzi {#sec-dynamics}
...
\newpage
## Retencja użytkowników {#sec-retention}
...
g7 = (ggplot(
    ...
    + xlab("Liczba wyświetleń (skala logarytmiczna)")
    + ylab("Wartość score")
    )
g8 = (ggplot(
    edge_questions_pd, 
...
## Analiza znaczników (tagów) {#sec-tags}
```{python}
...
tags_views_agg = regexp_extract_all(tags_views, r'<(\w+)>', 99, "tags", "tags_split", True)\
    ...
    .groupBy('tag')\
    .agg(
        f.sum("view_count").alias("liczba wyświetleń sumarycznie")
    )

tag_top_views = tags_views_agg.orderBy("liczba wyświetleń sumarycznie", ascending=False).limit(10)
...
tag_top_views_pd.columns = ["tag", "liczba wyświetleń sumarycznie"]
...
## Analiza tytułów postów {#sec-titles}
